...
class AnimationManager {
    ...
    /**
     * Anima la entrada de las cards con un fade-in simple
     */
    async animateCardsIn(cards) {
        ...
        // Animar entrada de cada card con fade-in simple
        for (let i = 0; i < cards.length; i++) {
            ...
            // Esperar antes de la siguiente card (delay más corto)
            if (i < cards.length - 1) {
                await this.delay(200); // Delay reducido a 200ms
            }
        }
...
class TwitchCalendar {
    ...
    init() {
        ...
        // Iniciar animación de entrada automáticamente al cargar
    ...
    async startEntryAnimation() {
        ...
        // Animación completada, las cards permanecen visibles
        this.updateAnimationStatus('Animación completada', 'completed');
        this.updateAnimationButtonState();
    }
    ...
    createDayCard(day, data) {
        ...
        const dayName = day.toUpperCase(); // En la imagen está todo en mayúsculas

        // Manejar imágenes múltiples o única
        let imageHtml = '';
        if (Array.isArray(data.image)) {
            // Si hay múltiples imágenes, usamos la primera por defecto o creamos un collage si se requiere
            // Para este diseño, parece mejor usar una imagen de fondo principal.
            // Usaremos la primera imagen como fondo.
            imageHtml = `<img src="${data.image[0]}" alt="Juego para ${dayName}" class="card__bg-image">`;
        } else {
            imageHtml = `<img src="${data.image}" alt="Imagen de portada para ${dayName}" class="card__bg-image">`;
        }
      ...
      <div class="card__inner">
        ...
        <div class="card__content">
          <div class="card__header">
            <h2 class="game-title">${data.game}</h2>
          </div>
    ...
    createTimeZones(times) {
        ...
        const otherCountries = [
            { code: 'MX', flagCode: 'mx', time: times.mexico },
            { code: 'AR', flagCode: 'ar', time: times.argentina },
            { code: 'CO', flagCode: 'co', time: times.colombia },
            { code: 'CL', flagCode: 'cl', time: times.chile }
        ];

        const otherTimesHtml = otherCountries.map(country => `
      <div class="mini-time-box">
        <div class="country-info">
          <img src="https://flagcdn.com/24x18/${country.flagCode}.png" alt="${country.code}" class="flag-img">
          <span class="country-code">[${country.code}]</span>
        </div>
        <div class="time-value">${country.time}</div>
      </div>
    `).join('');
      ...
      <div class="main-time-display">
        <div class="time-box">
          ...
          <span class="time-value">${spainTime}</span>
